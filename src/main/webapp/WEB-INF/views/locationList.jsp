...
function getLocationList() {	
    ...
    $('#engineerList').bootstrapTable('destroy')
	.bootstrapTable()
			...
			if(list != null) {
				if(list.length > 0) {
					for(var i=0; i < list.length; i++) {
						list[i].no = i+1;
						list[i].view_detail = '<div onclick="detailView('+list[i].role+',\''+list[i].uw_id+'\')"class="btn btn-default" style="background-color:transparent !important; padding:0; width:2.5vw; height:2.7vh; box-shadow:none;"><img src="images/company/ss/img_view.png" style="height:30px;"></div>';
					}				
					$('#engineerList').bootstrapTable('load', list );	
					$('#engineerList').bootstrapTable('hideLoading');
				}				
			}			
			else{
				$('#engineerList').bootstrapTable('load', [] );
				$('#engineerList').bootstrapTable('hideLoading');
			}
...
function detailView(role, u_id) {
	G_ROLE = role;
	G_UID = u_id;
	G_INPUT_DATE = $('#input_date').val();
	getLocationDetail();
	$('#location_log_modal').modal('show');
}

function getLocationDetail() {	
	
    $('#location_log_table').bootstrapTable('destroy')
	.bootstrapTable()
  	.bootstrapTable('showLoading')	
			...
			if(list.length > 0) {
				for(var i=0; i < list.length; i++) {					
					...
					if(list[i].section_name != "") {
						...
						list[i].section_info = '<div class="section"><span><i class="fa-regular fa-square"></i> #'+list[i].section+'</span></div>';
					}				
				...
				$('#location_log_table').bootstrapTable('load', list );	
				$('#location_log_table').bootstrapTable('hideLoading');
			}else{
				$('#location_log_table').bootstrapTable('load', [] );
				$('#location_log_table').bootstrapTable('hideLoading');
			}
		}
	});	
...
<div id="content-wrapper">
	<div id="content_title" class="content-item">위치파악로그</div>
	
	<div class="content_button_box content-item" >
		<div class="btn btn-primary" onclick="printRtls()"><i class="fa-solid fa-print"></i> 출력</div>
	</div>
	
	<div class="content_selete_box content-item">
		<form id="searchForm" class="form-contractor" action="recordList_renewal" method="POST" autocomplete="off" > 
	...
	<div class="content_table_box content-item">
		<table id="engineerList" data-loading-template="loadingTemplate" data-search="true" data-pagination="true" data-page-size="100" 
...
<div id="form_group">	
	<form id="printRtls" action="printRtls" method="POST">
...
<div style="display:none" id="modal_location_log" data-toggle="modal" data-target="#location_log_modal"></div>
<div class="modal fade" id="location_log_modal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
			...
			<div class="modal-body">
		        <div class="col-sm-12">
				    <div id="sub-tab-box" class="tab-content nav nav-tabs">
						<table id="location_log_table" data-loading-template="loadingTemplate" data-search="false" data-pagination="true"
						...
						<thead>
							<tr>							
	            				...
	            				<th class="text-center" data-field="section_info">위 치</th>
