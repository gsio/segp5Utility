...
var HOSTED_VIEWER_ORIGINS = ['null', 'http://localhost:8080/basement', 'http://seg.gsil.net'];
...
function webViewerInitialized() {
  ...
  //prompt("parseQueryString",queryString +"\n"+JSON.stringify(params));
  //prompt("webViewerInitialized",file);
...
function parseQueryString(query) {
  ...
  //prompt("parseQueryString",JSON.stringify(parts));
  for (var i = 0, ii = parts.length; i < ii; ++i) {
    var param = parts[i].split('=');
    var key = param[0].toLowerCase();

    var value = param.length > 1 ? param[1] : null;
    if(key == "file" && param.length > 1 && parts.length >= 2 ){
      value +=("=" +  param[2] + "&" + parts[1] + "&" + parts[2]);
    }
